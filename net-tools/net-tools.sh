...
class Socks5Server(SocketServer.StreamRequestHandler):
 ...
 def handle(self):
  print 'connection from ', self.client_address
  REMOTE_ADDR_STR=\"REMOTE_ADDR=\"+str(self.client_address[0])+\";\"
  REMOTE_ADDR_STR+=\"REMOTE_PORT=\"+str(self.client_address[1])+\";\"
  sock = self.connection
  Method=self.Read()
  GETSTR=self.Read()
  pip = os.popen(REMOTE_ADDR_STR+\" $SERVER_PROCESS_CMD \" + GETSTR  )
  while True:
   data=pip.read(1024)
   if data.__len__()==0 :
    pip.close()
